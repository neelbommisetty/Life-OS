...
| Auth | Neon Auth proxy passthrough | `ALL /api/auth`, `ALL /api/auth/*` | Proxies method/headers/body/query; strips hop/proxy forwarding headers before upstream auth calls; strips `content-type`/`content-length` for empty non-GET auth bodies to avoid upstream media-type rejection; forwards upstream status/headers; rewrites upstream auth redirect `Location` headers to proxy-relative `/api/auth/*`; base URL missing error |
